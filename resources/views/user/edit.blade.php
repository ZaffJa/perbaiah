...
<div class="widget ">
    ...
    <div class="widget-content">
        ...
        <form class="form-horizontal" method="post" action="{{ action('UserController@update', $user->id) }}" autocomplete="off" enctype="multipart/form-data">
            <fieldset>
                {{ csrf_field() }}
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input type="checkbox" class="toggle-value" {{ intval($user->gambar_ahli) == 1 ? 'checked' : null }} name="gambar_ahli" id="gambar_ahli" value="{{ $user->gambar_ahli }}">
                    </div>
                </div>
                <div class="control-group">
                    <label for="serahan_sijil" class="control-label">Serahan Sijil</label>
                    <div class="controls">
                        <input type="checkbox" class="toggle-value" {{ intval($user->serahan_sijil) == 1 ? 'checked' : '' }} id="serahan_sijil" name="serahan_sijil" value="{{ $user->serahan_sijil }}">
                    </div>
                </div>
                <div class="control-group">
                    <label for="serahan_kad_ahli" class="control-label">Kad Ahli</label>
                    <div class="controls">
                        <input type="checkbox" class="toggle-value" {{ intval($user->serahan_kad_ahli) == 1 ? 'checked' : '' }} id="serahan_kad_ahli" name="serahan_kad_ahli" value="{{ $user->serahan_kad_ahli }}">
                    </div>
                </div>
                <div class="control-group">
                    <label for="yuran_kad_ahli" class="control-label">Bayaran Yuran Kad Ahli RM10</label>
                    <div class="controls">
                        <input type="checkbox" class="toggle-value" {{ intval($user->yuran_kad_ahli) == 1 ? 'checked' : '' }} id="yuran_kad_ahli" name="yuran_kad_ahli" value="{{ $user->yuran_kad_ahli }}">
                    </div>
                </div>
                <div class="control-group">
                    <label for="yuran_masuk" class="control-label">Bayaran Yuran Masuk RM20</label>
                    <div class="controls">
                        <input type="checkbox" class="toggle-value" {{ intval($user->yuran_masuk) == 1 ? 'checked' : '' }} id="yuran_masuk" name="yuran_masuk" value="{{ $user->yuran_masuk }}">
                    </div>
                </div>
                <div class="control-group">
                    <label for="yuran_tabung_khairat" class="control-label">Bayaran Yuran Tabung Khairat dan Kebajikan RM15</label>
                    <div class="controls">
                        <input type="checkbox" class="toggle-value" {{ intval($user->yuran_tabung_khairat) == 1 ? 'checked' : '' }} id="yuran_tabung_khairat" name="yuran_tabung_khairat" value="{{ $user->yuran_tabung_khairat }}">
                    </div>
                </div>
...
<script>
    ...
    $('.toggle-value').on('change', function() {
        var currentToggleValue =  $(this);

        if (currentToggleValue.val() == 1) {
            currentToggleValue.val(0);
        }
        else {
            currentToggleValue.val(1);
        }
        console.log(currentToggleValue.val());
    })
